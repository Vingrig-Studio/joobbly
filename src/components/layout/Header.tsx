...
export function Header() {
    ...
    return <header className={`header ${overlaysHero ? "header--overlay" : ""} ${(overlaysHero ? heroPassed : scrolled) ? "header--scrolled" : ""} ${showTrial ? "header--trial-visible" : ""}`}><div className="container header-row"><Link className="logo" href="/" aria-label="Joobby — главная" onClick={() => setOpen(false)}><Image src={overlaysHero ? "/brand/joobby-mark-inverse.svg" : "/brand/joobby-mark.svg"} width={38} height={38} alt="" priority/><span>Joobby</span></Link><nav className="desktop-nav" aria-label="Основная навигация">{navigation.map(item => <Link key={item.href} href={item.href} aria-current={active(item.href) ? "page" : undefined}>{item.label}</Link>)}</nav><div className="header-actions"><ThemeToggle /><span className="lang" aria-label="Текущий язык: русский" title="Пока доступен русский язык">RU</span><a className="login" href={site.registrationUrl} target="_blank" rel="noopener noreferrer">Войти</a><Link className={`button button-small header-trial ${showTrial ? "is-visible" : ""}`} href="/pricing/" aria-hidden={!showTrial || undefined} tabIndex={showTrial ? undefined : -1} onClick={() => track("header_start_free_click")}>Попробовать бесплатно</Link><button ref={toggle} className="menu-toggle" type="button" aria-expanded={open} aria-controls="mobile-menu" aria-label={open ? "Закрыть меню" : "Открыть меню"} onClick={() => setOpen(!open)}><span /><span /></button></div></div><div ref={menu} id="mobile-menu" className={`mobile-menu ${open ? "is-open" : ""}`} inert={!open} aria-hidden={!open}><nav aria-label="Мобильная навигация">{navigation.map(item => <Link key={item.href} href={item.href} aria-current={active(item.href) ? "page" : undefined} onClick={() => setOpen(false)}>{item.label}<span aria-hidden="true">↗</span></Link>)}</nav><Link className="button" href="/pricing/" onClick={() => { setOpen(false); track("header_start_free_click"); }}>Попробовать бесплатно</Link></div></header>;
}
